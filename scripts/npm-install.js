...
/**
 * Postinstall script for the awsdac npm package.
...
if (!platform || !arch) {
  console.warn(
    `awsdac: Unsupported platform "${process.platform}/${process.arch}". ` +
...
const binaryName = isWindows ? 'awsdac.exe' : 'awsdac';
const nativeBinaryName = isWindows ? 'awsdac-bin.exe' : 'awsdac-bin';
const zipBaseName = `awsdac-${VERSION}_${platform}-${arch}`;
...
const tmpDir = path.join(os.tmpdir(), `awsdac-install-${Date.now()}`);
const zipPath = path.join(tmpDir, zipFileName);
const nativeBinaryPath = path.join(binDir, nativeBinaryName);

// Skip if binary already exists and matches current version
if (fs.existsSync(nativeBinaryPath)) {
  console.log(`awsdac: Binary already installed at ${nativeBinaryPath}`);
  process.exit(0);
}
...
console.log(`awsdac: Downloading ${downloadUrl} ...`);
...
download(zipPath, zipPath, (err) => {
  if (err) {
    cleanup();
    console.warn(`awsdac: Failed to download binary — ${err.message}`);
    console.warn('Install manually from: https://github.com/fernandofatech/diagram-as-code/releases');
    process.exit(0); // non-fatal: allow npm install to succeed
  }

  console.log(`awsdac: Extracting ${zipFileName} ...`);
  ...
  try {
    ...
    console.log(`awsdac: Binary installed at ${nativeBinaryPath}`);
    console.log(`awsdac: Run "awsdac --version" to verify.`);
  } catch (e) {
    console.warn(`awsdac: Failed to extract binary — ${e.message}`);
    console.warn('Install manually from: https://github.com/fernandofatech/diagram-as-code/releases');
  } finally {
    cleanup();
  }
